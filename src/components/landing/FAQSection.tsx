  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`flex h-6 w-6 items-center justify-center rounded-full shrink-0 transition-colors duration-200 ${open ? "bg-primary/15" : "bg-accent group-hover:bg-primary/10"}`}>
          {open ? (
...
const FAQSection = ({ dbContent }: { dbContent?: SectionContent<FAQContent> }) => {
  ...
  const leftFaqs = content.items.slice(0, 6);
        ...
        <div className="text-center mt-10 md:mt-14">
          <p className="text-sm text-muted-foreground">
            ...
            <a href="#contact" className="text-primary font-medium hover:underline underline-offset-4">
              {currentLang === "fr" ? "Parler à l'équipe →" : "Talk to our team →"}
            </a>
          </p>
        </div>
      </div>
