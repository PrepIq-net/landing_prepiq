            ...
            {content.posSystems?.map((pos, i) => {
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      background: isLive
                        ? "hsl(40 70% 39% / 0.15)"
                        : "hsl(220 14% 20% / 0.4)",
                      color: isLive ? meta.color : "hsl(220 14% 55%)",
                    }}
                  >
                    {meta.initial}
                  </div>
                  <div>
                    <p
                      className={`text-sm font-semibold ${isLive ? "text-foreground" : "text-muted-foreground/60"}`}
                    >
                      {pos.name}
                    </p>
                    <div className="flex items-center gap-1.5 mt-1">
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${isLive ? "bg-success" : "bg-muted-foreground/25"}`}
                      />
                      <span
                        className={`text-[11px] ${isLive ? "text-success" : "text-muted-foreground/40"}`}
                      >
                        {isLive
                          ? isFr
          ...
          >
            {tripledLogos.map((name, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={`text-[13px] sm:text-sm font-medium whitespace-nowrap ${isLive ? "text-foreground/80" : "text-muted-foreground/40"}`}
                  >
                    {name}
                  </span>
                  {isLive && (
