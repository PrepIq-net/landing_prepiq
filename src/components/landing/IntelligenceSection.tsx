...
const ALERT_META = [
  { icon: ArrowDown, color: "hsl(var(--warning))", bg: "hsl(var(--warning) / 0.08)", border: "hsl(var(--warning) / 0.15)", impact: "$14" },
  { icon: WarningTriangle, color: "hsl(var(--destructive))", bg: "hsl(var(--destructive) / 0.08)", border: "hsl(var(--destructive) / 0.15)", impact: "$38" },
  { icon: DollarCircle, color: "hsl(var(--info))", bg: "hsl(var(--info) / 0.08)", border: "hsl(var(--info) / 0.15)", impact: "$22" },
];
...
const IntelligenceSection = ({ dbContent }: { dbContent?: SectionContent<IntelligenceContent> }) => {
  const { t, i18n } = useTranslation();
  const currentLang = (i18n.resolvedLanguage || "en") as "en" | "fr";

  const content: IntelligenceContent = dbContent?.[currentLang] || {
    badge: t("intelligence.badge"),
    title: t("intelligence.title"),
    subtitle: t("intelligence.subtitle"),
    signals: [
      { label: t("intelligence.signals.sales.label"), desc: t("intelligence.signals.sales.desc") },
      { label: t("intelligence.signals.patterns.label"), desc: t("intelligence.signals.patterns.desc") },
      { label: t("intelligence.signals.weather.label"), desc: t("intelligence.signals.weather.desc") },
      { label: t("intelligence.signals.events.label"), desc: t("intelligence.signals.events.desc") },
      { label: t("intelligence.signals.stockouts.label"), desc: t("intelligence.signals.stockouts.desc") },
      { label: t("intelligence.signals.chef.label"), desc: t("intelligence.signals.chef.desc") },
    ],
    ...
    alerts: [
      { type: t("marginGuard.alerts.waste.type"), detail: t("marginGuard.alerts.waste.detail"), action: t("marginGuard.alerts.waste.action") },
      { type: t("marginGuard.alerts.stockout.type"), detail: t("marginGuard.alerts.stockout.detail"), action: t("marginGuard.alerts.stockout.action") },
      { type: t("marginGuard.alerts.leak.type"), detail: t("marginGuard.alerts.leak.detail"), action: t("marginGuard.alerts.leak.action") },
    ],
    atRiskLabel: currentLang === "fr" ? "à risque" : "at risk",
    suggestedLabel: currentLang === "fr" ? "Suggéré" : "Suggested",
    leakTypes: [t("marginGuard.leakTypes.overprep"), t("marginGuard.leakTypes.stockout"), t("marginGuard.leakTypes.batch"), t("marginGuard.leakTypes.drift")],
    totalProtectionLabel: t("marginGuard.totalProtection"),
    roiNote: t("marginGuard.roiNote"),
    whyBadge: t("whyPrepIQ.badge"),
    whyPoints: [
      { highlight: t("whyPrepIQ.points.0.highlight"), body: t("whyPrepIQ.points.0.body") },
      { highlight: t("whyPrepIQ.points.1.highlight"), body: t("whyPrepIQ.points.1.body") },
      { highlight: t("whyPrepIQ.points.2.highlight"), body: t("whyPrepIQ.points.2.body") },
    ],
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 sm:gap-2 mb-2">
                        ...
                        <span className="text-xs sm:text-sm font-semibold text-foreground">{meta.impact} {content.atRiskLabel}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 rounded-lg bg-accent/60 px-2.5 sm:px-3 py-1.5 sm:py-2">
                        ...
                        <span className="text-[11px] sm:text-[12px] text-foreground font-medium">{content.suggestedLabel}: {alert.action}</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm sm:text-base leading-relaxed text-muted-foreground">
                    <span className="font-semibold text-foreground">{p.highlight}</span> {p.body}
                  </p>
                </motion.div>
              );
            })}
