...
const BuiltForScaleSection = ({
  ...
  const content: BuiltForScaleContent = dbContent?.[currentLang] || {
    ...
    branches: [
      ...
      {
        name: "Dubai Marina",
        country: isFr ? "Émirats Arabes Unis" : "UAE",
        scenario: {
      ...
      },
      {
        ...
        country: "Nigeria",
        scenario: {
          tag: isFr ? "Protégé par le réseau" : "Network Protected",
          problem: "",
          aiLearning: "",
          prevention: isFr
            ? "Aucun incident ce mois-ci"
      ...
      {
        name: "Sydney",
        country: isFr ? "Australie" : "Australia",
        scenario: {
          tag: isFr ? "Protégé par le réseau" : "Network Protected",
          problem: "",
          aiLearning: "",
    ...
    globalFeatures: [
      {
        label: t("globalReady.features.currency.label"),
        desc: t("globalReady.features.currency.desc"),
      },
      {
        label: t("globalReady.features.timezone.label"),
        desc: t("globalReady.features.timezone.desc"),
      },
      {
        label: t("globalReady.features.localized.label"),
        desc: t("globalReady.features.localized.desc"),
      },
      {
        label: t("globalReady.features.support.label"),
        desc: t("globalReady.features.support.desc"),
      },
    ],
    regions: [
      t("globalReady.regions.na"),
      t("globalReady.regions.eu"),
      t("globalReady.regions.me"),
      t("globalReady.regions.af"),
      t("globalReady.regions.ap"),
      t("globalReady.regions.la"),
    ],
  };

  const branches = useMemo(() => {
    const branchItems = Array.isArray(content.branches) ? content.branches : [];
    return branchItems.map((b, i) => ({ ...b, ...BRANCH_META[i] }));
  }, [content]);
  const personas = Array.isArray(content.personas) ? content.personas : [];
  const globalFeatures = Array.isArray(content.globalFeatures)
    ? content.globalFeatures
    : [];
  ...
  const selectedBranch =
    branches.find((b) => b.name === activeBranch) || branches[0] || null;
      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-border/40 rounded-2xl overflow-hidden border border-border mb-16 sm:mb-24">
          {personas.map((p, i) => {
            ...
            return (
              <div
                key={p.title}
                className="bg-card p-4 sm:p-6 space-y-2 sm:space-y-3 hover:bg-accent/40 transition-colors duration-200"
              >
            ...
            <div className="h-[280px] sm:h-[360px] lg:h-[420px] w-full relative">
                    ...
                    <Popup className="prepiq-popup">
                      <div className="min-w-[180px]">
                        ...
                        <div
                          ...
                          style={{ color: "hsl(240 4% 56%)" }}
                        >
                          <span>{t("multiBranch.popup.accuracy")}</span>
                          <span style={{ color: "hsl(40 70% 50%)" }}>
                            {branch.accuracy}%
                          </span>
                        </div>
                      </div>
              ...
              <div className="absolute top-3 left-3 z-[500] flex items-center gap-2 rounded-lg border border-border/60 bg-card/80 px-3 py-1.5">
                ...
                <span className="text-[10px] sm:text-xs text-muted-foreground font-medium">
                  {t("multiBranch.liveBadge", { count: branches.length })}
                </span>
              </div>
            </div>
              ...
              <div className="divide-y divide-border/30">
                {branches.map((branch) => (
                  <button
                    ...
                    onClick={() =>
                      setActiveBranch(
                        activeBranch === branch.name ? null : branch.name,
                      )
                    }
        ...
        {selectedBranch && (
          <motion.div
            ...
            className="rounded-xl border border-border bg-card p-5 sm:p-6 mb-10 sm:mb-14"
          >
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <span className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <span>{selectedBranch.flag}</span> {selectedBranch.name}
              </span>
              <span className="text-[10px] uppercase tracking-wider font-semibold px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
                {selectedBranch.scenario.tag}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-border/40 rounded-xl overflow-hidden border border-border mb-16 sm:mb-24">
          {networkStats.map((stat) => (
            <div
              key={stat.label}
              className="bg-card px-4 sm:px-6 py-4 sm:py-6 text-center hover:bg-accent/40 transition-colors duration-200"
            >
              <p className="text-xl sm:text-2xl md:text-3xl font-semibold text-primary mb-1">
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
          {regions.map((name, i) => (
            <div
              key={name}
              className="flex items-center justify-center gap-2 sm:gap-2.5 rounded-full border border-border bg-card/80 px-3 sm:px-4 py-2 sm:py-2.5"
            >
