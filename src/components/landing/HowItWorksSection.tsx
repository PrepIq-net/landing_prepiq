...
const dataSignals = [
  { icon: DatabaseScript, label: "Historical sales" },
  { icon: Calendar, label: "Day-of-week patterns" },
  { icon: Clock, label: "Operating hours" },
  { icon: SunLight, label: "Events & holidays" },
  { icon: Cloud, label: "Weather signals" },
  { icon: UserStar, label: "Chef adjustments" },
  { icon: FlashOff, label: "Stockout history" },
  { icon: GraphUp, label: "Trend detection" },
];
...
const phases: { id: Phase; step: number; label: string; time: string; desc: string; icon: typeof SunLight; color: string; dot: string; bg: string }[] = [
  { id: "plan", step: 1, label: "Plan", time: "Morning", desc: "AI forecasts your prep", icon: SunLight, color: "text-[hsl(var(--success))]", dot: "bg-[hsl(var(--success))]", bg: "bg-[hsl(var(--success)/.08)]" },
  { id: "live", step: 2, label: "Monitor", time: "Service", desc: "Track sales in real-time", icon: Activity, color: "text-[hsl(var(--warning))]", dot: "bg-[hsl(var(--warning))]", bg: "bg-[hsl(var(--warning)/.08)]" },
  { id: "review", step: 3, label: "Learn", time: "Close", desc: "Review & improve", icon: StatsReport, color: "text-primary", dot: "bg-primary", bg: "bg-primary/[0.08]" },
];
      ...
      <div className="section-container px-4 sm:px-6">
        ...
        >
          ...
          <p className="text-sm sm:text-base text-muted-foreground max-w-xl mx-auto leading-relaxed">
            Three phases. One learning loop. Every day your kitchen gets smarter,
            wastes less, and earns more.
          </p>
        </motion.div>
        ...
        */}
        ...
        <AnimatePresence mode="wait">
          {activePhase === "plan" && <PlanPhase key="plan" />}
      ...
      <div className="space-y-6 sm:space-y-8">
        <div>
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-xs font-medium text-[hsl(var(--success))] uppercase tracking-widest">Morning</span>
          </div>
          <h3 className="text-2xl md:text-3xl font-semibold text-foreground mb-4 leading-tight">
            AI-Powered Prep Forecast
          </h3>
          <p className="text-muted-foreground leading-relaxed text-base">
            Before your kitchen opens, PrepIQ analyses data from your POS system
            to predict exactly how much of each item to prepare.
            No more gut feelings — just data-driven quantities with confidence scores.
          </p>
        </div>
        ...
        <div className="rounded-xl sm:rounded-2xl border border-border bg-card/80 backdrop-blur-sm p-4 sm:p-6 space-y-3">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-sm font-semibold text-foreground">Chef Override</p>
          </div>
          <p className="text-sm text-muted-foreground leading-relaxed">
            Your team can always adjust quantities. When they do, PrepIQ instantly
            shows the risk: potential waste cost, stockout probability, and profit
            upside. The AI learns from every override to improve tomorrow's forecast.
          </p>
        </div>
      </div>
      ...
      <PreviewCard
        topBar={
          <>
            <div className="flex items-center gap-2.5">
              <SunLight className="h-4 w-4 text-[hsl(var(--success))]" />
              <span className="text-sm font-medium text-foreground">Morning Forecast</span>
            </div>
            <span className="text-xs text-muted-foreground">Tuesday, Mar 5</span>
          </>
        }
      >
        {/* Forecast rows */}
        {[
          { item: "Grilled Salmon", qty: "25 kg", conf: 88, sold: "Tue avg: 24 kg" },
          { item: "Caesar Salad", qty: "40 portions", conf: 82, sold: "Tue avg: 38 portions" },
          { item: "Tomato Soup", qty: "15 L", conf: 91, sold: "Tue avg: 14 L" },
        ].map((row, i) => (
          <motion.div
            key={row.item}
              ...
              <div className="rounded-xl border border-[hsl(var(--warning)/.25)] bg-[hsl(var(--warning)/.05)] p-4 sm:p-5 space-y-3 sm:space-y-4">
                 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                   <div className="flex items-center gap-2.5">
                     ...
                     <span className="text-xs sm:text-sm font-medium text-foreground">Grilled Salmon → 30 kg</span>
                   </div>
                   <span className="text-[10px] sm:text-xs text-[hsl(var(--warning))] font-medium bg-[hsl(var(--warning)/.1)] px-2.5 py-1 rounded-md w-fit">+5 kg override</span>
                 </div>
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  {[
                    { label: "Waste risk", value: "$6.20", color: "text-destructive" },
                    { label: "Stockout risk", value: "Low", color: "text-[hsl(var(--success))]" },
                    { label: "Profit if right", value: "+$18.50", color: "text-primary" },
                    { label: "Override accuracy", value: "67%", color: "text-foreground" },
                  ].map((s) => (
                    <div key={s.label}>
                ...
                <div className="flex items-start gap-2.5 rounded-lg bg-accent/60 px-3 sm:px-4 py-2.5 sm:py-3">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Tuesday demand typically drops 8% vs Monday. Historical chef
                    overrides for this item are accurate 67% of the time.
                    <span className="text-primary font-medium"> AI will learn from this decision.</span>
                  </p>
                </div>
              </div>
...
const LivePhase = () => {
  const features = [
    { title: "Live depletion tracking", desc: "See remaining stock update in real-time as orders come in." },
    { title: "Stockout prediction", desc: "AI predicts when you'll run out based on current sell-through rate." },
    { title: "Smart alerts", desc: "Get notified before a stockout happens — not after." },
    { title: "Pace analysis", desc: "Compare today's pace vs forecast to spot deviations early." },
  ];
      ...
      <div className="space-y-6 sm:space-y-8">
        <div>
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-xs font-medium text-[hsl(var(--warning))] uppercase tracking-widest">Service</span>
          </div>
          <h3 className="text-2xl md:text-3xl font-semibold text-foreground mb-4 leading-tight">
            Real-Time Monitoring
          </h3>
          <p className="text-muted-foreground leading-relaxed text-base">
            Once service begins, PrepIQ tracks sales via your POS integration.
            See what's selling fast, what's lagging, and where risk is building — all in real time.
          </p>
        </div>
        ...
        topBar={
          <>
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-medium text-[hsl(var(--success))]">Live Service</span>
            </div>
            <span className="text-xs text-muted-foreground">2:34 PM · 68% through service</span>
          </>
        }
      >
        {[
          { item: "Grilled Salmon", sold: "18 kg", left: "12 kg", pct: 60, status: "on-track" as const, eta: "Enough until close" },
          { item: "Caesar Salad", sold: "32 portions", left: "8 portions", pct: 80, status: "warning" as const, eta: "May run out by 4 PM" },
          { item: "Tomato Soup", sold: "13 L", left: "2 L", pct: 87, status: "critical" as const, eta: "~25 min of stock left" },
        ].map((row, i) => (
          <motion.div
            key={row.item}
            initial={{ opacity: 0, y: 10 }}
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">{row.item}</p>
                  <p className="text-xs text-muted-foreground/60 mt-0.5">Sold: {row.sold}</p>
                </div>
              </div>
              <div className="text-right">
                ...
                <span className={`text-xs font-medium ${
                  ...
                  {row.status === "on-track" ? "On Track" : row.status === "warning" ? "Low Stock" : "⚠ Alert"}
                </span>
              </div>
            </div>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-foreground mb-1">Tomato Soup — Stockout Risk</p>
              <p className="text-sm text-muted-foreground leading-relaxed">
                At current pace, you'll run out in ~25 minutes. Consider notifying
                front-of-house or adjusting menu availability.
              </p>
            </div>
          </div>
        </motion.div>
...
const ReviewPhase = () => {
  const features = [
    { title: "Waste analysis", desc: "See exactly what was over-prepped and the financial impact." },
    { title: "Forecast vs actual", desc: "Item-by-item accuracy breakdown with trend over time." },
    { title: "Override scoring", desc: "Track how often chef overrides outperform the AI — and vice versa." },
    { title: "AI model update", desc: "Today's data is fed back to improve tomorrow's forecast automatically." },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="space-y-6 sm:space-y-8">
        <div>
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-xs font-medium text-primary uppercase tracking-widest">Close</span>
          </div>
          <h3 className="text-2xl md:text-3xl font-semibold text-foreground mb-4 leading-tight">
            Learn & Improve
          </h3>
          <p className="text-muted-foreground leading-relaxed text-base">
            After service, PrepIQ generates a full performance review: what was
            wasted, what stocked out, how accurate the forecast was, and what the
            chef overrides actually cost. Every data point sharpens tomorrow's predictions.
          </p>
        </div>
      ...
      <PreviewCard
        topBar={
          <>
            <div className="flex items-center gap-2.5">
              <StatsReport className="h-4 w-4 text-primary" />
              <span className="text-sm font-medium text-foreground">End of Day Report</span>
            </div>
            <span className="text-xs text-muted-foreground">Tuesday, Mar 5</span>
          </>
        }
      >
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          {[
            { label: "Waste Saved", value: "$38", sub: "vs. last week", color: "text-[hsl(var(--success))]" },
            { label: "Forecast Accuracy", value: "91%", sub: "↑3% from yesterday", color: "text-primary" },
            { label: "Stockout Events", value: "0", sub: "Target: <2", color: "text-[hsl(var(--success))]" },
            { label: "Revenue Protected", value: "$124", sub: "No missed sales", color: "text-primary" },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
        ...
        {/* Item breakdown */}
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-widest text-muted-foreground/60 font-medium">Item Performance</p>
          {[
            { item: "Grilled Salmon", forecast: "25 kg", actual: "23 kg", waste: "2 kg ($3.40)", accuracy: "92%" },
            { item: "Caesar Salad", forecast: "40 portions", actual: "38 portions", waste: "2 portions ($4.00)", accuracy: "95%" },
            { item: "Tomato Soup", forecast: "15 L", actual: "15 L", waste: "0 L", accuracy: "100%" },
          ].map((row, i) => (
            <motion.div
              key={row.item}
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] sm:text-xs text-muted-foreground">
                <span>Forecast: <span className="text-foreground font-medium">{row.forecast}</span></span>
                <span>Actual: <span className="text-foreground font-medium">{row.actual}</span></span>
        ...
        >
          ...
          <div>
            <p className="text-sm font-medium text-foreground">AI Model Updated</p>
            <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
              Today's data has been absorbed. Tomorrow's forecast will factor in
              today's patterns and override outcomes.
            </p>
          </div>
        </motion.div>
      </PreviewCard>
