...
export default async function ConnectorDetailPage({
  ...
  if (tab === "summary") {
    const [batchesRes, checkpointsRes] = await Promise.allSettled([
      ...
      djangoAdminFetch<SyncCheckpoint[]>(
    ...
    const [tablesRes, mappingsRes, checkpointsRes] = await Promise.allSettled([
      djangoAdminFetch<DiscoveredTable[]>(
        `/api/mgmt/connectors/${id}/schema/`,
        email,
      ),
      djangoAdminFetch<FieldMapping[]>(
        `/api/mgmt/connectors/${id}/field-mappings/`,
        email,
      ),
      djangoAdminFetch<SyncCheckpoint[]>(
        `/api/mgmt/connectors/${id}/checkpoints/`,
    ...
    heartbeatMs != null
      ? heartbeatMs < 60_000
        ...
        : heartbeatMs < 3_600_000
          ? `${Math.round(heartbeatMs / 60_000)}m ago`
          : heartbeatMs < 86_400_000
            ? `${Math.round(heartbeatMs / 3_600_000)}h ago`
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Records today",
            value: connector.records_synced_today.toLocaleString(),
          },
          {
            label: "Last sync",
                    ...
                    checkpoints.map((cp) => (
                      ...
                      >
                        ...
                        <TableCell className="px-6 py-3 text-[11px] font-mono text-muted-foreground whitespace-nowrap">
                          {cp.last_synced_at
                            ? new Date(cp.last_synced_at).toLocaleString()
                            : "—"}
                        </TableCell>
                        <TableCell className="px-6 py-3 text-[11px] font-mono text-muted-foreground whitespace-nowrap">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[11px] text-muted-foreground">
              Token values are hashed — only metadata shown. Use "Revoke All"
              above to invalidate.
            </p>
          </div>
          <div className="bg-[#1C1C1F] border border-[#2A2A2E] rounded-xl overflow-hidden">
            <Table>
              <TableHeader className="bg-[#232327]">
                <TableRow className="hover:bg-transparent border-b border-[#2A2A2E]">
                  {["Type", "State", "Expires", "Revoked At", "Created"].map(
                    (h) => (
                      <TableHead
                        key={h}
                        className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-6 py-3"
              ...
              <TableBody>
                {tokens.length === 0 ? (
                  ...
                  tokens.map((t) => {
                    const state = t.is_revoked
                      ? { label: "REVOKED", cls: "text-red-400" }
                      : t.is_expired
                        ? { label: "EXPIRED", cls: "text-muted-foreground" }
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="px-6 py-3">
                          <span className={`text-[11px] ${state.cls}`}>
                            {state.label}
                          </span>
                        </TableCell>
                        <TableCell className="px-6 py-3 text-[11px] font-mono text-muted-foreground whitespace-nowrap">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-display font-semibold text-foreground">
              Unreconciled Sales
              {unreconciledCount > 0 && (
                <span className="ml-2 text-red-400 text-sm font-mono">
                  ({unreconciledCount})
                </span>
              )}
            </h2>
          ...
          <div className="bg-[#1C1C1F] border border-[#2A2A2E] rounded-xl overflow-hidden">
            <Table>
              <TableHeader className="bg-[#232327]">
                <TableRow className="hover:bg-transparent border-b border-[#2A2A2E]">
                  {["Item", "Qty", "Amount", "Sale Time", "External ID"].map(
                    (h) => (
                      <TableHead
                        key={h}
                        className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-6 py-3"
